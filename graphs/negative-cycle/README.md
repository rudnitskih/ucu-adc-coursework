...
Both algorithms already have been implemented for the shortest path search. But for the task of negative cycle detection, we need slightly improve them and use custom implementations. Let's review each of them in details.

### Bellman-Ford implementation

Our implementation is based on pseudocode from Wikipedia. First of all, we noticed that this impelementation lacks early stopping point in the main loop. Our idea was to count the relaxed edges on a current iteration. If there were no relaxations in for one iteration, then it is clear that algorithm had found shortest paths for all vertices, visible from source. This changes slightly improved performance, but only for sparse graphs without negative cycles. In graphs with negative cycles, relaxation process is infinite, and our improvement has no influence on performance.

Second problem was graphs with more than one element of connectivity. Bellman-Ford is looking for shortest pash to the only one source vertex and thus will never relax edges invisible to the source. But we changed implementation to repeat algorithm relaxation process on every connectivity element. Therefore our implementation detects negative cycles in all isolated subgraphs and could also be used for counting of a graph connectivity elements.

### Floyd-Warshall implementation

Floyd-Warshall checks all possible paths from one vertice to another therefore it is free of Bellman-Ford problem with invisible edges. But it also could be changed to became more efficient in particular negative cycle detection problem. It turns out that it uses diagonal entries of the distance matrix as a detector of negative cycle presence after the main loop execution. But for early detection of negative cycle algorithm could stop main loop at the moment when any diagonal distance becomes negative because it clearly proves a presence of negative cycle. Therefore the performance of Floyd-Warshall could be drastically improved if it stops on the first negative cycle detected.
...
For both implementations we have found that in theory number of operations depends not only on a number of vertices but also on a connectivity between them. Therefore we have decided to test our algorithms on a two-dimensional set of input parameters. Other words, for each parameter `i` of the number of vertices we tested algorithms on 5 different graphs with connectivity parameter in set `0.0, 0.25, 0.5, 0.75, 1.0`. Here connectivity means a probability of having two arbitrary vertices in graph connected.
...
To minimize an influence of environment, we have run compiled program after reboot and have not started other processes. To reduce influence of multithreaded environment, explicitly set number of used threads to one by python instruction `mkl.set_num_threads(1)`. For time mesurement we used python liblary function `time.time()`
